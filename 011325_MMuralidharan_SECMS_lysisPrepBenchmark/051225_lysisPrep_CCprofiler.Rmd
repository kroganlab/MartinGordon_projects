...
Starting from peptide level info, no pre-filtering/ normalization etc prior to importing
Using default thresholds where possible to allow comparison 

Issues with. the input format files; for now try to hack the protein level data to work with the pipeline and just feed it in.. We can return to the peptide level quant later when the issues are addressed
```{r}
library(MSstats)
library(data.table)
library(ggplot2)
...
This is going to be our starting point... filter out the missing proteins
...
Use the calibration file to estimate protein monomer mw at elution and apply
```{r}
...
# calibrate ; seems ok.. why start so late in the column?
calibration <-  calibrateMW(cal.std) # also fits an lm to the table
```
...
filter to just proteins in the data
...
```
Now filter the proteins to the same set and produce a data.table of measurments 
```{r}
```
...
trace.list <- lapply(trace.list, function(x){
  ...
  # subset the mat to the set in the anno.dt
  submat <- mat[id %in% anno$id,]
  print(dim(submat))
  
  #match the row ord4ring 
...
unique(corum.dt$complex_name) %>% length()
```
...
# not great... looks like linear increase in positives above .05. I think 0.05 is a safer cutoff
qvalueComplexFeaturesScoredStats <- qvaluePositivesPlot(qvalueComplexFeaturesScored)

qvalueComplexFeaturesScoredStats
...
Seems fine; loop through each of the files, filter using standard thresholds and share with Ben
...
fread('~/Documents/projects/011325_MMuralidharan_SECMS_lysisPrepBenchmark/051225_lysisPrep_CCprofiler_data/2025_05_13_lysisPrep.ccprofiler.allComplexFeatures.csv.gz')
...
Notably, *CCprofiler* is able to retrieve co-elution evidence from full protein complex queries but also subsets thereof.
...
Thus, we collapse the initial results to truly unique signals with regard to (i), subunit composition and (ii), resolution in the chromatographic dimension.

```{r,eval=TRUE}

# this function clusters peaks, then loops through those peak clusters, performs complex clusteting within peaks (distance??) to get a unique feature identifiers, and assigns a unique id to each
complexFeaturesUnique <- getUniqueFeatureGroups(test,
                                                rt_height = 0,
                                                plot=T,
...
# collapses complexes to remove redundancy
complexFeaturesCollapsed[,.N, by=complex_name][N>1]
complexFeaturesCollapsed[complex_name == 'Rnase/Mrp complex']
...
No differential expression; just very quickly look at i) number of complexes in each group that pass and ii) the overlap in complex IDs (also maybe distribution of complex scores per sample)

```{r}

```

# these are peaks annotated to known complexes
```{r}
test[,.N, by=complex_id]
test[complex_id == 36]
...
**Not used**
Not suitable for use as we are not sure of the data quality
```{r}
...
```
Seems to be a lot of NA in the data; will need to split this into a list of 6 (one per sample) and create the correct input files for `CCprofiler`
```{r}
# so all the same rows are detected, but there are different missingness
lysisPrep.dt[is.na(Intensity), .N, by=.(Condition,BioReplicate)]


# summarize peptides with multiple features per peptide 
lysisPrep.dt[, sumIntensity := sum(Intensity, na.rm=T), by=.(Condition, BioReplicate, fraction, ProteinName, PeptideSequence)]
```

```{r}
lysisPrep.dt[!is.na(Intensity),.N, by=.(Condition, BioReplicate, fraction, ProteinName, PeptideSequence)][N>1]
```

```{r}
lysisPrep.dt[PeptideSequence == '_NKYEDEINKR_' & fraction == 7 & Condition == 'sonic' & BioReplicate == 1,]
```

```{r}
spec.list <- split(lysisPrep.dt, list(lysisPrep.dt$Condition, lysisPrep.dt$BioReplicate))
spec.list
```
Initial issue: duplicate intensity values per feature (intensity + precursor charge) in the data
```{r}
# peptide report
sonic <- fread('~/Library/CloudStorage/Box-Box/2025_02_SEC_Lysis_Test/Spectronaut/2.\ NP40-Lysis\ buffer\ +\ Sonication/20241227_104957_MM_Sonic_Lysis_122724_PEP_Report.tsv')
# msstats reprot
mss <- fread('~/Library/CloudStorage/Box-Box/2025_02_SEC_Lysis_Test/Spectronaut/2.\ NP40-Lysis\ buffer\ +\ Sonication/20250115_092735_MM_Sonic_Lysis_122724_MSStats_Report.txt')
# protein report
sonic.pg <- fread('~/Library/CloudStorage/Box-Box/2025_02_SEC_Lysis_Test/Spectronaut/2.\ NP40-Lysis\ buffer\ +\ Sonication/20241227_104957_MM_Sonic_Lysis_122724_PG_Report.tsv')
```

protein groups group report. Intensity values for `P02538` in fraction 7 in rep 1 and rep 2 sonic
```{r}
sonic.pg[PG.ProteinAccessions == 'P02538', .(PG.ProteinAccessions, `[7] Sonic-1_Frac-7.PG.Quantity`, `[79] Sonic-2_Frac-7.PG.Quantity`)]
```
peptide level report; intensity values for all peptides mapping to P02538 in fraction 7. 
These values are very different to the protein summarized level and are identical for all peptides 
```{r}
cols.oi <- c('PG.ProteinAccessions', 'EG.PrecursorId', grep('Sonic-1_Frac-7[.]', colnames(sonic), value=T))
sonic[PG.ProteinAccessions == 'P02538', ..cols.oi]
```
Other issues...
peptide data: precursor 2 reported for this protein even though not in above files? intensity values way off others too...
```{r}
sonic[ grepl('_NKYEDEINKR_', EG.PrecursorId), .(PG.ProteinAccessions, EG.PrecursorId, `[7] Sonic-1_Frac-7.PG.MS1Quantity`)]
```
msstats report also totally different to the two above..
```{r}
mss[ProteinName == 'P02538' &  Condition == 'Sonic'  & grepl('Frac-[7]$', Run),]
```

Big confusion with the input data.. I think we take the protein quant and see if we can 'hack' the pipeline to start from this level of info (no guarantees...)
```{r}
example.prot.traces
