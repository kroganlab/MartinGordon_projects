...
traces.subset <- enforceTraceAndAnnotationRowOrder(traces_obj = traces.obj)
```
...
f.anno.dt <-  prepareTracesFractionAnnotation(sec.long = sec.long[sample=='Infected_3'], mw_path = '/Users/martingordon/Documents/projects/061425_MMuralidharan_HIVdonor23_SECMS/data/HIV_Infection_CD4T_cells/SEC_Profiles/D1-D3-cal_SRT.txt')
trace.type <- 'protein'
...
Find the protein features; for this just duplicate the rows, will sum to get indv protein ints, but should be fine as just sums
Doesnt work; the differential analysis is on the peptide level; need to restart from peptides for this analysis
```{r}
# not sure if we need to duplicate these..
traces.subset$trace_annotation <- rbind(traces.subset$trace_annotation,
                                        traces.subset$trace_annotation)

traces.subset$traces <- rbind(traces.subset$traces,
                                        traces.subset$traces)

prot.elutions <- findProteinFeatures(traces = traces.subset, parallelized = TRUE, n_cores=8)
```
Get feature values; operates on peptide level, so use that info
```{r}
help(extractFeatureVals)

traces.pep <- copy(traces.subset)
traces.pep$trace_type <- 'peptide'
prot.features <- extractFeatureVals(traces.pep, prot.elutions, verbose = F)
```
```{r}
...
testDifferentialExpression(featureVals = prot.elutions,
...
help(extractFeatureVals)
help(testDifferentialExpression)

test.features <- extractFeatureVals(traces.subset, prot.elutions, verbose = F)
```
