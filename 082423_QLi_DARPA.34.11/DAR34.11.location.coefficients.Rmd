...
In this case the gene signatures we have do a nice job of seperating out the spatial signatures

```{r}
protQuant <- fread("./DARAP.34.11_data/2023_08_21_ProteinLevelData.tmp.2round.wCyto.csv")
...
hm <- Heatmap (spatRef.mat,
         cluster_rows = hc, column_names_gp = gpar(fontsize=7), column_split = tstrsplit(colnames(spatRef.mat), "_")[[1]]) +
  Heatmap (as.matrix(dcast(pq.subset[GROUP != "Cyto" & Protein %in% rownames(spatRef.mat)], Protein~GROUP+SUBJECT, value.var = "LogIntensities"), rownames = "Protein"), cluster_columns = FALSE, row_names_gp = gpar(fontsize=6), column_names_gp = gpar(fontsize=7))


BackupAsPDF(hm, 'location.coef.heatmap', dimensions=c(10,12))
```
...
Run the location deconvolution
...
Tidy the output for visualisation 
```{r}

repFitsLong

repFitsLong[, receptor := str_extract(group, "MRX2|NK1R")]
repFitsLong[, drug := 'SP'] %>% .[group %like% 'MRX2_R[0-9]{2}', drug := 'CZ']
repFitsLong[, bigGroup := paste0(receptor,'.',drug)]
repFitsLong[, timeStr := str_extract(group, '[0-9]{2}$')]
...
repFitsLong.median <- repFitsLong[,.(coefficient = median(coefficient)),by = .(rep,bg, bigGroup, time )]
...
repFitsLong.median.simple <- repFitsLong.median[, .(coefficient = mean (coefficient), stderr = standard.error(coefficient),
                                                    ...
                                                    maxCoef = max(coefficient)), by = .(bg, bigGroup, time)]
...
p
BackupAsPDF (p,'location.coefficients.random')
...
BackupAsPDF (p,'location.coefficients.noCyto.random')
